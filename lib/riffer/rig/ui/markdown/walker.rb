...
module Riffer::Rig::UI::Markdown::Walker
  ...
  INDENT_STEP = '  ' #: String
  BULLET = '• ' #: String
  ESCAPE_RE = /(?:\e\[[0-9;]*m|.)/m #: Regexp
  ANSI_RE = /\e\[[0-9;]*m/ #: Regexp
  SMART_QUOTES = {
  ...
  # @rbs highlighter: _Highlighter
  # @rbs return: Array[String]
  def render(source, width:, theme:, highlighter:)
    return [] if source.empty?

    tail = Riffer::Rig::UI::Markdown::UnstableTail.split(source)[1] # : String
    stable = tail.empty? ? source : source[0...(-tail.length)] # : String

    lines = [] # : Array[String]
    children = Kramdown::Document.new(stable, input: 'GFM').root.children #: Array[_KramdownElement]
  ...
  def wrap_styled(line, width)
    ...
    line.scan(ESCAPE_RE).each do |piece|
      piece = piece.to_s
      piece_width = piece.start_with?("\e") ? 0 : 1 # : Integer
  ...
  # @rbs item: _KramdownElement
  # @rbs width: Integer
  # @rbs theme: Riffer::Rig::UI::Theme
  # @rbs highlighter: _Highlighter
  # @rbs first: String
  # @rbs continuation: String
  # @rbs return: Array[String]
  ...
  # @rbs children: Array[_KramdownElement]
  # @rbs theme: Riffer::Rig::UI::Theme
  # @rbs highlighter: _Highlighter
  # @rbs return: String
  ...
  def header_text(block, theme)
    ...
    text = render_inline(block.children, theme: plain, highlighter: plain) # steep:ignore
