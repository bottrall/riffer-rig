...
- **Syntax**: per-param `@rbs` lines only — `# @rbs name: Type` for each parameter (never a leading `?` on the name: it's invalid rbs-inline that is silently ignored, typing the param `untyped`; optionality comes from the Ruby default), then `# @rbs return: Type`. Never `#:`. Use `# @rbs @ivar: Type` for instance variables, `# @rbs!` for standalone `@rbs! name: Type` embeds, and a trailing `#: Type` assertion on the same line for constants and `attr_reader`s (rbs-inline 0.14 only honours those there — an annotation on a preceding line is silently ignored).
